//TODO -> Button picks, button game7
//f11->122
...
(function($){
var fuliando = $.Event("keydown", { keyCode: 122});    
var cont=0;
...
if (navigator.userAgent.match('Chrome')) nativeFullScreenSupport = true;
...
FlareVideo.fn.r15 = function(){
  //this.seek((this.video.getCurrentTime())-15);
  //FlareVideo.seek(this.video.getCurrentTime()-15);
  //this.seek(40);
  this.video.setCurrentTime(this.video.getCurrentTime()-16); //modified from 15 to 16 to optimize the xperience
  //this.video.playbackRate -= .25;
};
FlareVideo.fn.faster = function(){
  this.video.playbackRate += .25;
};
FlareVideo.fn.rnormal = function(){
  this.video.playbackRate = 1;
};
FlareVideo.fn.slower = function(){
  this.video.playbackRate -= .25;
};
FlareVideo.fn.a15 = function(){
  //this.seek((this.video.getCurrentTime())+15);
  //FlareVideo.seek(this.video.getCurrentTime()+15);
  //this.seek(40);
...
FlareVideo.fn.sPick = function(){
   this.video.setCurrentTime(segPicks); 
};

FlareVideo.fn.sGame = function(){
    this.video.setCurrentTime(segGame);
};
...
//    }
  case 'mozGecko': {
      check=cont%2;
      if(check==0){
      document.getElementById("video").className = "videoFull";
      cont++;
      toggleFullScreen();
      //document.dispatchEvent(keyboardEvent);
       //jQuery.event.trigger({ type : 'keypress', which : character.charCodeAt(122) });
       //$("body").trigger(fuliando);
       //this.mozRequestFullScreen();
        }
      if(check==1){
        document.getElementById("video").className = "videoNormal";  
        cont++;
        toggleFullScreen();
        //this.mozRequestFullScreen();
        //jQuery.event.trigger({ type : 'keypress', which : character.charCodeAt(122) });
...
FlareVideo.fn.setupButtons = function(){ //TODO
  ...
  var faster = $("<div />");  //TOCHECK
  faster.addClass("faster");
  faster.text("faster");
  faster.click($.proxy(function(){
    this.faster();
  }, this));
  this.controls.append(faster);
  
    var rnormal = $("<div />");  //TOCHECK
  rnormal.addClass("rnormal");
  rnormal.text("rnormal");
  rnormal.click($.proxy(function(){
    this.rnormal();
  }, this));
  this.controls.append(rnormal);
  
    var slower = $("<div />");  //TOCHECK
  slower.addClass("slower");
  slower.text("slower");
  slower.click($.proxy(function(){
    this.slower();
  }, this));
  this.controls.append(slower);
  ...
  var sPick = $("<div />");
  sPick.addClass("sPick");
  sPick.text("sPick");
  sPick.click($.proxy(function(){
    this.sPick();
  }, this));
  this.controls.append(sPick);
  
  var sGame = $("<div />");
  sGame.addClass("sGame");
  sGame.text("sGame");
  sGame.click($.proxy(function(){
    this.sGame();
  }, this));
  this.controls.append(sGame);
